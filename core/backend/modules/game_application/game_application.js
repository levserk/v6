...
module.exports = class Engine {
    ...
    constructor(gameManager, conf) {
        ...
        this.eventBus = this.server.eventBus;
    ...
    onMessage(type, room, user, data) {
        switch (type) {
            ...
            case Engine.USER_TIMEOUT:
                return this.onTimeout(room, user, data.online);
            default:
    ...
    onUserReady(room, user, data) {
        ...
        return co(function* () {
            ...
            yield self.gm.sendUserReady(room, { user: user.userId, ready: data });
    ...
    onUserTurn(room, user, turn) {
        ...
        return co(function* () {
            ...
            if (!userTurn || typeof userTurn !== "object") { // wrong turn
                ...
                yield self.gm.sendError(game, user.userId, 'wrong_turn');
            ...
            userTurn = room.saveUserTurn(userTurn, nextPlayerId);

            yield self.gm.sendUserTurn(room, user.userId, userTurn);
    ...
    onTimeout(room, user, online) {
        let self = this, game = room.game;
        return co(function* () {
            user = yield self.memory.getUser(game, room.currentId);
            yield self.onUserTimeout(room, user, online);
        });
    }

    onUserTimeout(room, user, online){
        log(`onUserTimeout`, `user: ${user.userId}, online: ${online}`);
        room.timeout = false;
    ...
    onDraw(room, user, event) {
        let self = this, userId = user.userId, askDrawId = room.gameData.askDraw;
        return co(function* () {
            switch (event.action){
                case 'ask':
                    if (!askDrawId) {
                        room.gameData.askDraw = userId;
                        for (let playerId of room.players) {
                            if (playerId !== userId){
                                yield self.gm.sendUserEvent(room, playerId, {
                                    user: user.userId,
                                    type: 'draw',
                                    action: 'ask'
                                });
                        ...
                        if (askDrawId !== userId) { // already asked
                            log('onDraw', `auto draw, ${userId}, ${askDrawId}`, 2);
                            return self.onRoundEnd(room, {action: 'draw'});
                        }
                    }
                    break;
                case 'cancel':
                    if (askDrawId && askDrawId !== userId) {
                        room.gameData.askDraw = null;
                        return self.gm.sendUserEvent(room, askDrawId, {
                            user: userId,
                            type: 'draw',
                            action: 'cancel'
                        });
    ...
    onUserGameEvent(room, user, event) {
        let self = this;
        return co(function* () {
            event.target = user.userId;
            room.saveUserEvent(event, user.userId);
            yield self.gm.sendUserEvent(room, user.userId, event);
        });
    }
    ...
    onRoundEnd(room, result) {
        ...
        return co(function* () {
            ...
            let players = [];
            //TODO: warn user not exists here if he leaved game
            for (let playerId of room.players) {
                let user = yield self.memory.getUser(game, playerId);
                if (!user) {
